...
The database URL always comes from the environment (``DATABASE_URL``), never
from ``alembic.ini``, so migrations can be pointed at a throwaway CI database
without any committed credentials.
...
def get_database_url() -> str:
    ...
    if not url:
        raise RuntimeError(
            "DATABASE_URL is not set. Export it (or pass -x db_url=...) before "
            "running Alembic."
        )
    return url
